...
import { createTeacher } from '@/lib/firebase/api';
...
const ManageTeachers = () => {
	...
	// functions
	// form definition
	const form = useForm<z.infer<typeof TeacherValidation>>({
	...
	useEffect(() => {
		...
		onSnapshot(collection(db, 'teachers'), (ss) => {
			const tarr = ss.docs.map((doc, index) => (
				// <li id={doc.id}>
				<>
					{index > 0 && <Separator />}
	...
	return (
		<div className="flex flex-col w-full h-full p-10 box-border">
			...
			<div className="flex flex-row mb-5">
				<h1 className="font-bold text-lg">Manage Teacher</h1>
			</div>
